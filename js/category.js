    ...
    nextProducts.forEach(prod => {
        ...
        div.innerHTML = `
            ...
            <a href="${prodUrl}" style="display: block; overflow: hidden;">
                <img src="${categoryImgUrl}" alt="${escapeHTML(fullNameTitle)}" class="product-img" loading="lazy" style="transition: transform 0.5s ease;">
            </a>
            <div class="product-info">
                ...
                <div id="product-action-${prod.id}" class="product-action-container" data-name="${escapeHTML(fullNameTitle)}" data-price="${pricing.finalPrice}" data-original-price="${pricing.originalPrice}" data-discount-percent="${pricing.discountPercent}" data-img="${categoryImgUrl}" data-stock="${prod.stock !== undefined && prod.stock !== null ? prod.stock : ''}" data-limit="${prod.maxLimit !== undefined && prod.maxLimit !== null ? prod.maxLimit : ''}">
                </div>
            </div>
        `;
